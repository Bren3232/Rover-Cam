...
# root.iconbitmap('test-icon.ico')     # adding icon messing up scroll
...
#     if ask == True:
#         run(["cp", rc_dir + "/RC_config.txt", path_usb])
#         run(["cp", rc_dir + "/aoi_pts.txt", path_usb])
...
# going to make so RC all files can live on USB or pi, path_usb can = rc_dir
...
#         ao_im = resize(ao_i, height=plimg_h)
...
#     sett["use_disable_button"] = s7_txt.get("1.0", "end-1c")
    ...
    if sett["delay_audio"] == "":
        sett["delay_audio"] = "0"
    if sett["disable_button_presses"] == "":
        sett["disable_button_presses"] = "1"
    if sett["disable_button_time_frame"] == "":
        sett["disable_button_time_frame"] = "1"
    if sett["lock_disable_time"] == "":
        sett["lock_disable_time"] = "0"
    if sett["start_delay"] == "":
        sett["start_delay"] = "0"
