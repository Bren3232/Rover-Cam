...
#         print(yr, mo, da, hr, mi, se)
        ...
        if "y" in i[10:].lower():
            play = True
            audio = run(['ls', '/home/pi/Desktop/Rover_Cam/audio'], capture_output=True).stdout
...
# 
# print("motion_area is: ", motion_area)
# print("lock_disable_time is: ", lock_disable_time)
...
# print("audio discovered from folder is: " + audio)
# print("audio_delay discovered from folder is: " + str(delay_audio))
...
#     global delay_aud
    ...
    is_playing = True
    tt = time.time()
    while time.time() < tt + delay_aud:
        time.sleep(0.1)
        if time.time() > tt + delay_aud:
            for i in aud:      # plays one after other, all .wav and .py files in folder in alphabetical order
                if ".wav" in i:  # mp3, flac did not play well, only use .wav
                    run(['omxplayer', f'/home/pi/Desktop/Rover_Cam/audio/{i}'])
#                     run(['aplay', f'/home/pi/Desktop/Rover_Cam/audio/{i}'])
#    no work        run(['sudo', '-u', '"#1000"', 'XDG_RUNTIME_DIR=/run/user/1000', 'aplay', f'/home/pi/Desktop/Rover_Cam/audio/{i}'])
                if ".py" in i:
                    run(['python3', f'/home/pi/Desktop/Rover_Cam/audio/{i}'])
    is_playing = False
...
def disable():
    global bbp
    bbp += 1
#     print("bbp is ", bbp, " And bb_presses is ", bb_presses)
...
#                     print("ex = true from disable")
                    ex = True

                # ---- run await renable py file here -----
                    run(['python3', '/home/pi/Desktop/Rover_Cam/disable.py'])

                else:
                    # Log in the clips folder a txt file when a disable attempt fails
                    fa_num += 1
                    fa = open(sdate + f"/failed-btn-code-attempt-{fa_num}.txt", "a")
                    ...
                    fa.write("\n")
                    fa.write(f"In mode 2, on run: {sdate}")
                    fa.close()
...
s = open("/home/pi/Desktop/Rover_Cam/comm.txt", "r")
start = float(s.readline())
s.close()
# print("start in mode2 is: ", start)
m = 60

ran = random.randint(10000, 99000)

sdate = f"{yr}-{mo}-{da}-{hr}-{mi}-{se}"
sdate = path_usb + "/" + sdate + "_" + str(ran)
run(['mkdir', sdate])
